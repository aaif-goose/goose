...
export function assignedCoreTeamMembers(assignees, coreTeamByGithub) {
  return (assignees || [])
    .map((assignee) => assignee?.login)
    .filter(
      (login) =>
        typeof login === "string" && coreTeamByGithub.has(login.toLowerCase()),
    );
}

export function readyTransitionCoreTeamMember(
  events,
  { coreTeamByGithub, projectNumber, projectOwner },
) {
  const readyTransitions = events
    .filter(
      (event) =>
        event?.status === "Ready" &&
  ...
  if (readyTransitions.length === 0) {
    return { reason: "linked issue has no Ready transition in the project" };
  }

  const transition = readyTransitions[0];
  const login = transition.actor?.login;
  if (transition.wasAutomated || !login) {
    return { reason: "the latest Ready transition has no human actor" };
  }
  const person = coreTeamByGithub.get(login.toLowerCase());
  return person
    ? { person }
