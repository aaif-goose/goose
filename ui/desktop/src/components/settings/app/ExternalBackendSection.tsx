...
export default function ExternalBackendSection() {
  ...
  // switch-backend already swapped this window onto the external backend, so a
  // failed connection has to be undone or the window is left with no usable backend.
  const revertToLocalBackend = async () => {
    ...
    try {
      ...
      console.error('Failed to reconnect to the local backend:', error);
    }
  };
  ...
  const connect = async () => {
    ...
    setOutcome(null);
    try {
      // The renderer's connect-src only names the backend once it is enabled, so
      // this has to be saved before the socket is opened or CSP blocks it.
      ...
      try {
        ...
        await revertToLocalBackend();
        setOutcome({
          message: intl.formatMessage(i18n.connectFailed, { error: errorMessage(error) }),
          ok: false,
        });
        return;
      }

      setOutcome({ message: intl.formatMessage(i18n.connected), ok: true });
    } finally {
  ...
  const disconnect = async () => {
    ...
    try {
      ...
      try {
        ...
        setOutcome({
          message: intl.formatMessage(i18n.disconnectFailed, { error: errorMessage(error) }),
          ok: false,
        });
        return;
      }

      setOutcome({ message: intl.formatMessage(i18n.disconnected), ok: true });
    } finally {
      setIsConnecting(false);
    }
  };
  ...
  const buttonLabel = config.enabled
    ? isConnecting
      ? i18n.disconnecting
      : i18n.disconnect
    : isConnecting
