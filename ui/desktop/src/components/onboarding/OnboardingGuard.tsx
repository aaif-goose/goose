...
export default function OnboardingGuard({ children }: OnboardingGuardProps) {
  ...
  const changeBackend = async (switchTo: 'external' | 'local') => {
    setIsRetrying(true);
    try {
      const result = await (switchTo === 'external'
        ? window.electron.switchBackend()
        : window.electron.disconnectBackend());
      if (!result.ok) {
      ...
      try {
        ...
        setExternalBackendError(errorMessage(error));
        return;
      }
      ...
      <div className="h-screen w-full bg-background-default flex flex-col items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <div className="flex items-center justify-center gap-2">
            <Button onClick={() => changeBackend('external')} disabled={isRetrying}>
              {intl.formatMessage(isRetrying ? i18n.retrying : i18n.retry)}
            </Button>
            <Button variant="outline" onClick={() => changeBackend('local')} disabled={isRetrying}>
