...
Reference: `ui/talos/docs/diagnostics-plan.md`. Files changed in Phase 1:

- `ui/talos/src-tauri/Cargo.toml` — added `zip`, `chrono`, `os_info`.
- `ui/talos/src-tauri/src/commands/diagnostics.rs` *(new)*
- `ui/talos/src-tauri/src/commands/mod.rs` — registers the new module.
- `ui/talos/src-tauri/src/lib.rs` — two new commands in `invoke_handler!`.
- `ui/talos/src/services/diagnostics.ts` *(new)*
- `ui/talos/src/components/diagnostics/DiagnosticsModal.tsx` *(new)*
- `ui/talos/src/components/chat/Composer.tsx` — bug button `onClick`.
- `ui/talos/src/App.tsx` — state, modal mount, palette wiring, `onReportBug` prop.
...
`cargo check` failed during implementation with `Access is denied` inside `tauri-build`. If you hit the same, close any running Talos/Tauri process and delete the stale build artifacts:

```bash
# Close all Talos windows first.
taskkill /IM talos-tauri.exe /F
taskkill /IM talos.exe /F
# Optional nuke if builds still fail:
rm -rf ui/talos/src-tauri/target/debug/build/talos-*
```
...
**Pass criteria:** Talos window opens, no red errors in the Tauri dev console, no warnings about unknown commands.
...
1. Open Talos. Verify the **bug icon** is visible at the right edge of the composer footer (after MCP).
...
1. Start a fresh Talos. Do NOT send any messages.
2. Open the bug modal.
3. Click **Download**. The native save-dialog appears.
4. **Default filename check:** expect `talos-diagnostics-YYYYMMDD-HHMMSS-<6chars>.zip` where the tail is the last 6 chars of `tab.id`. Timestamp should be local-time "now" within a minute.
...
Labels on the issue draft (right sidebar): **bug, talos**.
...
If you see that, add `opener:allow-open-url` (with a URL scope for `https://github.com/**`) to `ui/talos/src-tauri/capabilities/default.json` and rebuild. Include a screenshot of the error in the test report.
...
1. Open the modal. Click Download. While the save-dialog is open, DO NOT touch the Talos window — OS dialog owns focus.
2. After the dialog resolves, click outside the Talos modal: it should close normally.
