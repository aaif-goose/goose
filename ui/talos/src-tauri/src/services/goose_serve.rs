...
/// at `/acp`. One process serves any number of concurrent sessions.
...
impl GooseServeProcess {
    ...
    async fn spawn(app_handle: tauri::AppHandle) -> Result<GooseServeProcess, String> {
        ...
        log::info!(
            "Spawning goose serve: binary={binary_display} port={port} cwd={}",
            working_dir.display(),
        );
...
/// Resolve the `goose` binary. `GOOSE_BIN` env overrides; otherwise use the
/// Tauri sidecar declared in `tauri.conf.json > bundle > externalBin`.
