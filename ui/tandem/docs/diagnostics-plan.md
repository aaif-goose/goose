# Tandem Diagnostics / "Report a Problem" Implementation Plan
...
- **Goose subprocess logs are discarded.** `ui/tandem/src-tauri/src/services/goose_serve.rs` (lines 53-55) currently has `stdout(Stdio::null())` and `stderr(Stdio::null())`. Log capture must be added as part of this feature.
...
## 1. Inventory — Desktop vs. Tandem

| Concern | Desktop (source) | Tandem (destination) | Action |
|---|---|---|---|
| Modal UI | `ui/desktop/src/components/ui/Diagnostics.tsx` | — | **Adapt** (strip i18n, lucide, `Button` primitive; use Tandem's `Icon` + `.scrim`/`.palette` CSS pattern from `SettingsModal.tsx`) |
| Bug-report trigger | `ui/desktop/src/components/ChatInput.tsx` (L1684-1710) | `ui/tandem/src/components/chat/Composer.tsx` L193-196 (no-op) | **Wire up** (lift `bugModalOpen` into `App.tsx` like `settingsOpen`, or keep local in `Composer` — see §3) |
| ZIP generator (Rust) | `crates/goose/src/session/diagnostics.rs::generate_diagnostics` — reads `Paths::in_state_dir("logs")`, `config.yaml`, `SessionManager::export_session`, templates | — | **Rebuild from scratch** in Tandem (no `Paths`, no `config.yaml`, no `session_manager`). Transcript flows from frontend, not Rust. |
| System info | `crates/goose/src/session/diagnostics.rs::SystemInfo` (uses `sys_info`, `chrono`, `Config::global`, `get_enabled_extensions`) | — | **Slim port**: only `os`, `os_version`, `architecture`, `app_version`, tandem build id, provider/model (provided by frontend), ACP port. No config/extensions concept. |
| ZIP HTTP delivery | `crates/goose-server/src/routes/status.rs` `GET /diagnostics/{id}` | — | **Not applicable.** Tandem has no goose-server HTTP. ZIP is built and saved directly via Tauri command + `plugin-dialog` save-dialog. |
...
| Config snapshot | `config.yaml` | Tandem `settings.json` in `$APP_CONFIG_DIR` | **Include** the Tandem settings file (paths, no secrets today). Also include `state.json` (the `tauri-plugin-store` file) *optionally* or with PII scrub. |
...
## 2. Rust work (`ui/tandem/src-tauri/`)
...
pub struct SystemInfo {
    app_version: String,          // env!("CARGO_PKG_VERSION") of tandem crate
    app_name: String,             // "Tandem"
...
- `logs/tandem-app.log` — if we adopt `tauri-plugin-log` (optional, phase 4).
- `memory/…` — **only** if `include_memory_dir` is true and `settings.memoryDir` resolves; walked with a 50 MB total-size cap.
- `README.txt` — boilerplate: "This bundle was generated by Tandem vX on [date]. Do not post publicly if your transcript contains secrets."

### New file: `src/services/log_capture.rs`

Responsibilities:

- Open/create a log file at `app_handle.path().app_log_dir()?.join("goose-serve.log")`. On Windows this resolves to `%LOCALAPPDATA%\com.tandem.app\logs\`.
...
- List items: "Basic system info", "Current session messages", "Recent Goose logs", "Tandem settings", "UI state snapshot", plus a checkbox for "Include memory folder (may contain personal notes)" — unchecked by default.
...
1. Call `save` from `@tauri-apps/plugin-dialog` with `defaultPath: \`tandem-diagnostics-${tabId}-${isoDate}.zip\`` and filter `[{ name: "Zip", extensions: ["zip"] }]`.
...
4. `URLSearchParams` with `template=bug_report.md`, `body`, `labels=bug,tandem`.
...
- Download the Tandem diagnostics zip from the same dialog and attach it below.
...
**Environment**
- **App:** Tandem ${info.appVersion} (Tauri ${info.tauriVersion})
...
- Path: `app_handle.path().app_log_dir()?` → on Windows that's `C:\Users\<u>\AppData\Local\com.tandem.app\logs\goose-serve.log`, on macOS `~/Library/Logs/com.tandem.app/`. On Tauri 2 you must call `std::fs::create_dir_all` since `app_log_dir` doesn't create it.
- On spawn:
  1. Create the dir.
...
### Rust (`ui/tandem/src-tauri/Cargo.toml`)
...
- Add Tandem `settings.json` + `state.json` copies to ZIP (with trivial PII pass — strip `composer` drafts, keep message bodies).
...
- Not shipped: the ZIP already surfaces goose's logs, and Tandem-side structured logging is thin. Pick this up when there's a real debugging need.
...
1. **ZIP filename format.** Proposal: `tandem-diagnostics-{YYYYMMDD-HHMMSS}-{shortTabId}.zip` where `shortTabId = tab.id.slice(-6)`. Avoids collisions when a user files multiple reports. *Decision needed.*
...
### Phase 3
- Manual: start Tandem, send a message (causing goose-serve activity), click Download → open the ZIP → `logs/goose-serve.log` exists and is non-empty. Kill Tandem, relaunch, confirm `.log.1` shows up.
...
- `ui/tandem/src-tauri/src/services/goose_serve.rs`
- `ui/tandem/src-tauri/src/lib.rs`
- `ui/tandem/src-tauri/Cargo.toml`
- `ui/tandem/src/App.tsx`
- `ui/tandem/src/components/chat/Composer.tsx`
