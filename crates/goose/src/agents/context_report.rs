...
impl Agent {
    pub async fn build_context_report(
        ...
        let auto_compaction_pending = crate::context_mgmt::check_if_compaction_needed(
            provider.as_ref(),
            &persisted,
            None,
            &session,
        ...
        Ok(ContextReportResponse {
            ...
            segments,
            provider_manages_context: false,
            auto_compaction_pending,
        })
    }
}
...
/// Tools are counted in request order as marginal additions to a growing
/// prefix so the parts sum to exactly what `count_tokens_for_tools` charges
/// the request; owners appear in the order their first tool does.
fn tool_segments(tools: &[Tool], token_counter: &TokenCounter) -> Vec<ContextSegment> {
    let mut segments: Vec<ContextSegment> = Vec::new();
    let mut segment_by_owner: HashMap<String, usize> = HashMap::new();
    ...
    for tool in tools {
        ...
        let index = *segment_by_owner.entry(owner.clone()).or_insert_with(|| {
            segments.push(ContextSegment {
        ...
        segments[index].token_count += part.token_count;
        segments[index].parts.push(part);
    }
    for segment in &mut segments {
        segment.source = Some(format!("{} tools", segment.parts.len()));
    }
    segments
}
...
mod tests {
    ...
    async fn tool_segments_account_for_every_tool_token_exactly() {
        ...
        let owners: Vec<&str> = segments
            .iter()
            .map(|segment| segment.label.as_str())
            .collect();
        assert_eq!(owners, ["ungrouped", "beta", "alpha"]);
